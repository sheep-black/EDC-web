...
const handleSearch = async () => {
  ...
  loading.value = true;
  try {
    const response = await axios.get(`/searchSMILESDX?smiles=${keyword}&pagenum=${currentPage.value-1}&pagesize=${pageSize.value}`);
...
<template>
  ...
  <div style="justify-content: center;display: flex; margin-top: 10px;">
    <el-table v-if="showTable"
              ...
              stripe
              max-height="600"
              :data="tableData.value" border style="width: 800px;"
      ...
      <el-table-column prop="edc" label="EDC or not" width="150"></el-table-column>
    </el-table>
  </div>
  <el-pagination
